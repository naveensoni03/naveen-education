...
export default function FeesLedger() {
  ...
  useEffect(() => {
    setTransactions([
      { id: "REC-2025-089", type: "Tuition Fee", student: "Naveen Soni", roll: "101", class: "10-A", total: 8500, paid: 8500, due: 0, date: "2025-12-22", status: "Paid", mode: "Cash", breakdown: { tuition: 5000, transport: 2000, library: 500, fine: 0 } },
      { id: "REC-2025-088", type: "Hostel Fee", student: "Rahul Singh", roll: "103", class: "12-Science", total: 15000, paid: 5000, due: 10000, date: "2025-12-20", status: "Partial", mode: "UPI", breakdown: { tuition: 15000, transport: 0, library: 0, fine: 0 } },
      { id: "INV-2025-001", type: "Library Fine", student: "Amit Sharma", roll: "105", class: "10-B", total: 500, paid: 0, due: 500, date: "2025-11-30", status: "Overdue", mode: "-", breakdown: { tuition: 0, transport: 0, library: 0, fine: 500 } },
    ]);
  }, []);
  ...
  const handleCollectFee = () => {
    ...
    setFormData({ studentName: "", rollNo: "", class: "10-A", feeType: "Tuition Fee", amount: "", paymentMode: "Cash", referenceNo: "", date: new Date().toISOString().split('T')[0], tuitionFee: "", transportFee: "", libraryFee: "", fine: "", discount: "" });
  };
        ...
        <div className="glass-container fade-in-up" style={{ flex: 1, padding: '30px', animationDelay: '0.4s', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{display: 'flex', gap: '10px', marginBottom: '20px', borderBottom: '1px solid #f1f5f9', paddingBottom: '15px'}}>
                {["All", "Paid", "Pending", "Overdue"].map(tab => (
                    <button key={tab} onClick={() => setFilter(tab)} className={`tab-btn-modern ${filter === tab ? 'active' : ''}`}>
            ...
            <div className="overlay-blur" onClick={() => setActivePanel("none")}>
                <div className={`luxe-panel slide-in-right ${activePanel === 'receipt' ? 'receipt-panel' : ''}`} onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="panel-header-simple no-print" style={{borderBottom: '1px solid #f1f5f9', paddingBottom: '20px', marginBottom: '20px'}}>
                        <div>
                            <h2 style={{margin: '0 0 5px', color: '#0f172a', fontWeight:'800'}}>
                                {activePanel === 'collect' ? 'Collect Fee' : activePanel === 'create' ? 'Create Invoice' : 'Payment Receipt'}
                            </h2>
                            <p style={{margin: 0, color: '#64748b', fontSize: '0.9rem'}}>
                                {activePanel === 'collect' ? 'Auto-filled based on class structure.' : activePanel === 'create' ? 'Generate new invoice manually' : `Ref: ${selectedTxn?.id}`}
                            </p>
                        </div>
                    ...
                    <div className="panel-content-scroll">
                        ...
                        {activePanel === 'create' && (
                            <>
                                <div className="input-group"><label>Student Name</label><input type="text" placeholder="e.g. Rahul Kumar" value={formData.studentName} onChange={(e) => setFormData({...formData, studentName: e.target.value})} style={inputStyle} /></div>
                                <div className="grid-2-col">
                                    <div className="input-group"><label>Class</label><input type="text" value={formData.class} onChange={(e) => setFormData({...formData, class: e.target.value})} style={inputStyle} /></div>
                                    <div className="input-group"><label>Fee Type</label><select value={formData.feeType} onChange={(e) => setFormData({...formData, feeType: e.target.value})} style={inputStyle}><option>Tuition Fee</option><option>Hostel Fee</option><option>Transport Fee</option><option>Library Fine</option></select></div>
                                </div>
                                <div className="input-group"><label>Amount (₹)</label><input type="number" value={formData.amount} onChange={(e) => setFormData({...formData, amount: e.target.value})} style={inputStyle} /></div>
                                <button className="btn-confirm-gradient hover-lift" onClick={handleCreateInvoice} style={{width: '100%', padding: '16px', fontSize: '1rem', marginTop:'10px'}}>Generate Invoice</button>
                            </>
                        )}
